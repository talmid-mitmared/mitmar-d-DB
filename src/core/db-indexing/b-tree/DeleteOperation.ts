...
import {
  borrowKeyFromChildren,
  rebalanceLeafOperation,
  rebalanceOperation,
} from './Balance';
import {
  $BtPage,
...
export function deleteInTreeV2(page: $BtPage, queryKey: number) {
  ...
  while (!deleted) {
    ...
    if (queryHit && index != null) {
      ...
      if (isInternal) {
        const { end: childEnd } = searchInPage(
  ...
  while (unbalanced) {
    ...
    if (isPageOverflows(rebalanced) === false) {
      ...
      return createBtreePage({
        ...rootPage!.node,
        children: rootPage!.node.children.map((c, i) =>
          i === rootPage!.index ? rebalanced : c,
        ),
      });
    }
  }
