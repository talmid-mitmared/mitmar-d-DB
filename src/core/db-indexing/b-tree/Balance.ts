...
export function rebalanceLeafOperation(queryKey: number, page: $BtPage) {
  const ableToBorrowFromChildren = !isBothSiblingsWillUnderflows(
    queryKey,
    page,
  );
 ...
 */
export function balanceFromBorrowSiblings(
  queryKey: number,
  parentPage: $BtPage,
): $BtPage {
   ...
   */
  const parentPageWithDebt = borrowKeyFromChildren(
    rightSiblingIndex,
    leftSiblingIndex,
    parentPage,
  );
   ...
   */
  const parentPageWithNoDebt = borrowKeyFromParent(
    currentIndex,
    parentPageWithDebt,
  );
 ...
 */
export function balanceFromMergeSiblings(
  currentIndex: number,
  parentPage: $BtPage,
): $BtPage {
  /**
  ...
  const mergedPage = mergeSiblingPages(
    ...
    getRelativeIndex(
      currentIndex,
      parentPage.children[currentIndex].recordKeys.length,
    ),
...
export function borrowKeyFromChildren(
  ...
  const canborrow = (borrowingIndex: number) =>
    doNotCheckUndeflows === false
      ? canborrowFn(borrowingIndex)
      : doNotCheckUndeflows;
...
export function atomicBorrowFromChild(
  targetIndex: number,
  childIndex: number,
  page: $BtPage,
) {
  const { page: updatedPage, value } = extractKeyInPage(
    childIndex,
    page.children[targetIndex],
  );
...
export function atomicBorrowFromParent(targetIndex: number, page: $BtPage) {
  ...
  const insertedChildPage = insertInPage(
    updatedPage.children[targetIndex],
    value,
  );
...
export function rebalanceOperation(
  queryKey: number,
  page: $BtPage,
  parentPage: $BtPage,
) {
  ...
  const ableToBorrowFromChildren =
    isBothSiblingsWillUnderflows(childIndex, page) === false;
